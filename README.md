...
The public development benchmark work is collected in [schema_matching_public_dev_v1.json](data/benchmarks/schema_matching_public_dev_v1.json), with separate fixture notes for [Open Food Facts](data/benchmarks/development/open_food_facts_products_v1/README.md), [UK Contracts Finder](data/benchmarks/development/contracts_finder_procurement_2026_v1/README.md), and the [combined public development results](data/benchmarks/development/combined_public_dev_v1/comparison.json). The combined public development corpus has 35 cases and 25 expected links. Baseline, V4, and V5 currently have the same aggregate metrics on those two public development benchmarks, so this is useful coverage but not a V5 win claim.
...
### Identifier-role detection and negative ranking ablation

The [value-profile identifier detector](data/experiments/schema_matching_identifier_role_v1/README.md) asks whether aggregate source column values can identify fields with an identifier role without using source headers, target identity, scenario identity, or ground-truth-derived ranking features. On 107 development fields, with 36 identifier and 71 non-identifier labels, leave-one-contract-family-out validation reached balanced accuracy 0.8326, precision 0.7778, recall 0.7778, F1 0.7778, ROC-AUC 0.9163, and AP 0.8883. The combined logistic model beat constant, heuristic, distribution-only, and pattern-only baselines, but this remains development-only evidence rather than a production promotion. See the [detector module](src/core/mapping/identifier_role_detection.py).

The detector was then used in an independent [identifier-role ranking ablation](data/experiments/schema_matching_identifier_role_ranking_ablation_v1/README.md) to test whether role-compatible reranking should be added to V5. The ablation used nested leave-one-contract-family-out evaluation, selected alpha only from inner grouped out-of-fold predictions, and left V5 itself unchanged. Both heuristic and learned rerankers selected `alpha=0` in every outer fold; V5, heuristic, learned, and oracle systems had identical pooled ranking metrics, with improved/regressed cases at 0/0. Because even the oracle role signal produced no ranking lift, there is no evidence to connect this signal to V5 or runtime. The project keeps this negative result instead of tuning alpha after the fact to manufacture an improvement. See the [reranker module](src/core/mapping/identifier_role_reranker.py).

The FDIC sealed banking evidence is split across independent commits: the [protocol](data/benchmarks/sealed/fdic_bankfind_locations_v1/protocol.json) preregistered the holdout, the [guarded runner](scripts/evaluate_fdic_bankfind_sealed_benchmark.py) enforced single execution, and the [first sealed result](data/benchmarks/sealed/fdic_bankfind_locations_v1/first_evaluation.json) records the one completed evaluation. Baseline, V4, and V5 ranking all scored Top-1 2/3 on the three target-bearing fields. Both the existing V5 policy and the score-only 95% calibrator accepted 0/14 cases, while all 11 no-target cases were safely rejected. The development coverage improvement did not reproduce on this sealed banking source, so the calibration remains negative evidence, not a production promotion. See the [FDIC sealed README](data/benchmarks/sealed/fdic_bankfind_locations_v1/README.md) for the all-review outcome and limitations.
